...
const independentEvents = computed((): EventEntry[] =>
...
const events = computed((): EventEntry[] =>
  independentEvents.value.filter(e => eventEntryMatchesCodeFilter(e, traces.eventCodeFilterText))
)
...
<template>
  <main class="traces-main">
    ...
    <div v-if="independentEvents.length === 0" class="empty-state">
    ...
    <div class="trace-header-bar">
      ...
      <span class="trace-event-count">{{ events.length }} / {{ independentEvents.length }} events</span>
